...
## Spend Ingest Pipeline

### Overview

A 4-stage Cloudflare Worker (`apps/workers/spend-ingest/`) ingests UK public sector transparency spending data (over £25k/£500 CSV/ODS reports). Runs **weekly** via cron (Monday 3AM UTC), processing 200 buyers per invocation with cursor-based resume.
...
Central government departments (MoD, HMRC, etc.) each have their own spending publication pages — there is NO single cross-department collection page. The pipeline handles this with:

1. **`GOVUK_DEPT_SLUG_MAP`** (`patterns/transparency-urls.ts`): Maps buyer names → GOV.UK URL slugs (e.g., `"ministry of defence"` → `"mod"`). Probes department-specific paths like `/government/publications/mod-spending-over-25000-january-to-december-{year}`.
2. **`filterLinksToBuyer()`** (`stages/01-discover.ts`): Filters collection page links using `DEPT_ABBREVIATION_MAP` with slugified name keywords + known abbreviations. Fallback: returns all links if zero match.
3. **`followGovukPublicationPages()`** (`stages/02-extract-links.ts`): Resolves `/government/publications/` HTML pages → actual ODS/CSV download URLs from `assets.publishing.service.gov.uk/media/`.

### File Format Support

Stage 3 supports three formats via `detectFileFormat(contentType, url)`:
- **CSV**: `text/csv`, `text/plain`, `application/csv` → PapaParse
- **ODS**: `application/vnd.oasis.opendocument.spreadsheet`, `.ods` extension → SheetJS (`xlsx` library)
- **XLSX**: `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`, `.xlsx` extension → SheetJS

Both paths produce identical `{ data: Record<string, string>[]; headers: string[] }` for downstream normalization.

### Known Column Schemas

`normalization/known-schemas.ts` — 11 schemas ordered by specificity:

| Schema | Detects | Used By |
|--------|---------|---------|
...
| + 7 more generic patterns | Various | Other UK councils + NHS |

AI fallback via Claude Haiku when no schema matches. Cached in-memory by sorted headers hash.

### MongoDB Collections (Spend)

| Collection | Purpose |
|------------|---------|
| `spendtransactions` | Individual spend records per buyer. Compound dedup: buyerId + date + vendor + amount + reference. Max 5,000 per buyer. |
| `spendsummaries` | Pre-computed per-buyer aggregates: total spend, category/vendor/monthly breakdowns, date range, source files. |
...
Runs all 4 stages for one buyer. Limit 3 files in test mode. Inlines worker logic to avoid import path issues.

### Verified Results (MoD)
...
- Stage 3: 1,499 transactions from 3 monthly ODS files, £2.2B total
- Stage 4: 17 categories (Defence Equipment, Infrastructure, Strategic Command, etc.), 50+ vendors
