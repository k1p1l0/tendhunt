...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/components/contracts/contract-filters.tsx"
      provides: "Sector, region, and value range filter dropdowns updating URL params"
      contains: "use client"
  ...
  key_links:
    ...
    - from: "src/components/contracts/contract-filters.tsx"
      to: "URL searchParams"
      via: "useRouter().replace() with sector/region/value params"
      pattern: "replace.*pathname.*params"
...
2. `fetchContracts(filters: ContractFilters)` async function:
   ...
   - Execute three queries in parallel with `Promise.all`:
     - `Contract.find(query).sort({ publishedDate: -1 }).skip((page - 1) * pageSize).limit(pageSize).select('-rawData').lean()`
   ...
   - NOTE: When `$text` is combined with other filters, MongoDB uses the text index for the `$text` part and applies the other conditions as post-filters. The explicit `.sort({ publishedDate: -1 })` overrides text score sorting, which is the desired behavior for a chronological feed.
...
**Create `src/components/contracts/contract-filters.tsx`** -- Client component ('use client'):
   ...
   - Render three Select dropdowns in a flex row with gap-2:
...
**Replace `src/app/(dashboard)/contracts/page.tsx`** -- Server component:
   ...
   - Destructure: `query`, `sector`, `region`, `minValue`, `maxValue`, `page` (all string | undefined)
   - Parse page to number (default 1), minValue/maxValue to number if present
   - Call `fetchContracts({ query, sector, region, minValue, maxValue, page, pageSize: 20 })`
   - Render:
  ...
  <done>
    Contracts page shows a searchable, filterable, paginated feed of contract cards. Search debounces at 300ms. Filters for sector (20 options), region (14 options including UK-wide and Unspecified), and value range (6 predefined brackets). Pagination with Previous/Next. Count display shows total and filtered. All URL-param driven for shareability and back/forward navigation. Empty state handled.
  </done>
</task>
...
- 3 filter dropdowns (sector, region, value) are combinable
