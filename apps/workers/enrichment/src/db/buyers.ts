 ...
 * If fields contains __addLinkedinSource: true, adds "linkedin" to enrichmentSources via $addToSet.
 */
export async function bulkUpdateBuyerEnrichment(
  ...
  const ops = updates.map((u) => {
    const { __addLinkedinSource, ...setFields } = u.fields;
    ...
    if (__addLinkedinSource) {
      update.$addToSet = { enrichmentSources: "linkedin" };
    }
    return {
      updateOne: {
