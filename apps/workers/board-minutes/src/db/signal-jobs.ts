...
import type { SignalJobDoc, SignalIngestStage } from "../types";

const COLLECTION = "signalingestjobs";
...
): Promise<SignalJobDoc> {
  const collection = db.collection<SignalJobDoc>(COLLECTION);

  const existing = await collection.findOne({ stage });
  if (existing) return existing;

  const now = new Date();
  const newJob: SignalJobDoc = {
    stage,
    status: "running",
    cursor: null,
    batchSize: 50,
