...
const scoreBadgeRenderer: CustomRenderer<ScoreBadgeCell> = {
  ...
  draw: (args, cell) => {
    ...
    const { score, isLoading, isQueued, cacheKey } = cell.data;
    ...
    if (cacheKey) {
      ...
      if (!scoreCompletionTime.has(cacheKey)) {
        ...
        // Memory management: clean up oldest entries when cache grows large
    ...
    // Subtle glow ring during pop
    if (scale > 1.01) {
      ctx.beginPath();
      ctx.arc(cx, cy, animRadius + 3, 0, Math.PI * 2);
      ctx.strokeStyle = color;
      ctx.globalAlpha = 0.3 * (scale - 1) / 0.2;
      ctx.lineWidth = 2;
      ctx.stroke();
      ctx.globalAlpha = 1;
    }
