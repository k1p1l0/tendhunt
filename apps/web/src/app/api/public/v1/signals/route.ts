...
export async function GET(request: Request) {
  ...
  if (searchParams.get("organizationName"))
    input.organizationName = searchParams.get("organizationName");
  if (searchParams.get("signalType"))
    input.signalType = searchParams.get("signalType");
  ...
  try {
    const result = await executeToolHandler("query_signals", input, auth!.userId);
